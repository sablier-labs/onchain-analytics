...
SELECT
    ...
    from_hex(json_extract_scalar(commonparams, '$.broker')) AS broker,
    cast(json_extract_scalar(commonparams, '$.cancelable') AS boolean) AS cancelable,
    contract_address,
    evt_block_number,
    evt_block_time,
    evt_index,
    evt_tx_hash,
    from_hex(json_extract_scalar(commonparams, '$.funder')) AS funder,
    from_hex(json_extract_scalar(commonparams, '$.recipient')) AS recipient,
    from_hex(json_extract_scalar(commonparams, '$.sender')) AS sender,
    streamid,
    ...
    CASE
        WHEN
            from_hex(json_extract_scalar(commonparams, '$.token')) IN (
            ...
            THEN cast(
                cast(
                    json_extract_scalar(json_extract_scalar(commonparams, '$.amounts'), '$.deposit') AS double
                ) AS double
            ) / 1e6
        WHEN
            from_hex(json_extract_scalar(commonparams, '$.token')) IN (
    ...
    json_extract_scalar(json_extract_scalar(commonparams, '$.timestamps'), '$.start') AS starttime,
    json_extract_scalar(json_extract_scalar(commonparams, '$.timestamps'), '$.end') AS endtime,
    (
        cast(
            json_extract_scalar(json_extract_scalar(commonparams, '$.timestamps'), '$.end') AS double
        ) - cast(
            json_extract_scalar(json_extract_scalar(commonparams, '$.timestamps'), '$.start') AS double
        )
    ) AS duration,
    clifftime AS cliffduration,
    json_extract_scalar(json_extract_scalar(commonparams, '$.amounts'), '$.deposit') AS deposit,
    '0' AS protocolfee,
    json_extract_scalar(json_extract_scalar(commonparams, '$.amounts'), '$.brokerFee') AS brokerfee
FROM
    sablier_lockup_v2_0_multichain.sablierlockup_evt_createlockuplinearstream
...
SELECT
    ...
    json_extract_scalar(json_extract_scalar(commonparams, '$.amounts'), '$.brokerFee') AS brokerfee
FROM
    sablier_lockup_v2_0_multichain.sablierlockup_evt_createlockupdynamicstream
...
SELECT
    ...
    json_extract_scalar(json_extract_scalar(commonparams, '$.amounts'), '$.brokerFee') AS brokerfee
FROM
    sablier_lockup_v2_0_multichain.sablierlockup_evt_createlockuptranchedstream
